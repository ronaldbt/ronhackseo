<template>
  <div style="width: 600px; max-height: 800px;" class="overflow-y-auto bg-gray-50 font-sans">
    <header class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white px-5 py-5 flex justify-between items-center shadow-lg">
      <h1 class="text-xl font-semibold m-0 flex items-center gap-2">
        <MagnifyingGlassIcon class="w-5 h-5" />
        SEO Extension Pro
      </h1>
      <button @click="analyze" class="bg-white text-indigo-600 border-none px-5 py-2.5 rounded-lg font-semibold cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0" :disabled="loading">
        {{ loading ? 'Analizando...' : 'Analizar Página' }}
      </button>
    </header>

    <div v-if="loading" class="flex flex-col items-center justify-center py-16 px-5">
      <div class="w-10 h-10 border-4 border-gray-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
      <p class="text-gray-600 font-medium">Analizando página...</p>
    </div>

    <div v-else-if="pageData" class="p-5 space-y-5">
      <!-- Score Dashboard -->
      ...
      <section class="bg-white rounded-xl p-5 shadow-md hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5">
        ...
        <SchemaDetector :schemas="pageData.schemas || []" :recommendations="pageData.schemaRecommendations || []" />
      </section>
  ...
  <div>
    ...
    <div v-else class="py-16 px-5 text-center text-gray-500">
      <p class="text-base">Haz clic en "Analizar Página" para comenzar</p>
    </div>
  </div>
</template>
...
const analyze = async () => {
  ...
  try {
    ...
    // Verificar si estamos en una página especial
    ...
    // Intentar primero comunicarse con el content script existente
    try {
      chrome.tabs.sendMessage(tab.id, { action: 'getPageData' }, async (response) => {
        if (chrome.runtime.lastError) {
          // Si no hay respuesta, intentar inyectar el código directamente
          console.log('Content script no disponible, inyectando código...')
          try {
            ...
            if (result && result[0] && result[0].result) {
              pageData.value = result[0].result
            } else {
              alert('No se pudieron obtener los datos de la página')
            }
          } catch (injectError) {
            console.error('Error al inyectar código:', injectError)
            loading.value = false
          ...
          loading.value = false
          alert('No se recibió respuesta del content script')
        }
      })
    } catch (error) {
    ...
    loading.value = false
    alert('Error al analizar la página. Asegúrate de que la extensión tenga permisos.')
  }
}
</script>
